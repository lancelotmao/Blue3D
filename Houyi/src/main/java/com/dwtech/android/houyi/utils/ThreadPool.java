...
public class ThreadPool {
    ...
    public <T> Future<T> submitUrgent(Job<T> job, FutureListener<T> listener) {
        ThreadPoolExecutor excutor = (ThreadPoolExecutor)mExecutor;        
        BlockingQueue<Runnable> queue = excutor.getQueue();
        Worker<T> w;
        Future<T> res;
        int size = queue.size();
        res = submit(job, listener);
        for (int i = 0; i < size; i++) {
            w = (Worker<T>)queue.poll();
            if (w != null) {
                excutor.remove(w);
                excutor.execute(w);
            }
    ...
    public <T> Future<T> submitToLast(Future<T> future) {
        ThreadPoolExecutor excutor = (ThreadPoolExecutor)mExecutor;        
        BlockingQueue<Runnable> queue = excutor.getQueue();
        Worker<T> w = (Worker<T>)future;
