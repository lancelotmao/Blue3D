...
//    [dataArray addObject:@"Sci-fi"];
    ...
    UIBarButtonItem* save = [[UIBarButtonItem alloc] initWithTitle:@"Menu" style:UIBarButtonItemStylePlain target:self action:@selector(openMenu)];
...
- (void)applySetting
{
    bool isOn = [BlueSetting getInstance].mShowGrd;
    if (mGrid) {
...
- (void)updateSettingUI
{
    ...
    [mSwitchGrid setOn:[BlueSetting getInstance].mShowGrd];
...
{
    BOOL state = [sender isOn];
    [BlueSetting getInstance].mShowGrd = state;
    if (mGrid) {
...
- (void)shouldGotoSingle
{
    ...
    dispatch_async( dispatch_get_main_queue(), ^{
        ...
        canvas->drawImageCropCenter(ImageFactory::createImage(mCapturedImage));
